...
## Минимальный запуск обязательной части

В этой версии реализована только обязательная часть лабораторной: PostgreSQL, Spark и ClickHouse.
...
bash spark/run_job1.sh
bash spark/run_job2.sh
```

Проверка загрузки исходных данных в PostgreSQL:

```bash
docker-compose exec postgres psql -U lab -d lab2 -c "select count(*) from mock_data;"
```

Проверка модели звезды в PostgreSQL:

```bash
docker-compose exec postgres psql -U lab -d lab2 -c "select count(*) from fact_sales;"
```

Проверка отчетов в ClickHouse:

```bash
docker-compose exec clickhouse clickhouse-client -u lab --password lab --database lab2 --query "show tables"
docker-compose exec clickhouse clickhouse-client -u lab --password lab --database lab2 --query "select * from report_product_sales order by total_revenue desc limit 10"
```
